...
\begin{figure}[H]
  ...
  \caption{
    \new{
      ...
      except that BD was used to generate the twin tree.
...
\new{The candidate model that had highest evidence given the simulated alignment,
...
The strict clock model matches the model used to simulate the alignment.
The BD model is perhaps somewhat surprising for the true tre because the other alteranative standard tree prior, Yule, is probably closest to the true DD model because it shows no pull-of-the-present (but also no slowdown).
}
