...
# Create the inference model for the generative model explicitly
...
# Simulate a twin tree using BD explicitly
...
\begin{figure}[H]
  ...
  \caption{
    \new{The inference error made 
    when using the generative substitution and clock models (in this case JC69 and strict clock) and a Yule tree prior (this is the assumed generative model because the real generative model is of course unknown) and when using the best-fitting candidate model (in this case JC69, relaxed log-normal and BD), for both the true tree generated by the novel diversification model and for the twin tree.
...
we can see that the error distributions of the \new{ (assumed) generative model (i.e. the known generative substitution and clock models, and the tree model that is used for generating and inferring the twin tree and assumed in inference of the true tree)}
