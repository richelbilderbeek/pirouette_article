...
\begin{figure}[H]
  ...
  \caption{
    \new{The impact of the tree prior for the example tree in Figure 2. The alignment for this true tree was generated using a JC69 substitution model and strict clock model. For inferring the tree from this alignment in the 'generative' scenario the same substitution and clock models were used, and a Yule tree prior (this is the assumed generative model because the real generative model is of course unknown). In the 'best' scenario the best-fitting candidate models (JC69 substitution model, relaxed log-normal clock model and BD tree prior) were used. For the twin tree, the same models were chosen, except that BD was used to generate and infer the twin tree in the 'generative' scenario.
