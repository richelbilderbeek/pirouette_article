...
\new{as well as coalescent priors.}
...
Plugins have been provided, for instance, for the calibrated Yule model [\cite{heled2015calibrated}],
...
depends on one or more traits [\cite{maddison2007estimating, fitzjohn2012diversitree, herrera2018detecting} ].
...
\rampal{This only increases the confusion; maybe replace by baseline model everywhere}
\new{
...
\giovanni{Do we need the library calls in the listing?}
...
The DD tree generated by this code is shown in Figure \ref{fig:dd_tree}. \iffalse % wordcut (this info is already in listing)
...
\giovanni{I don't understand this sentence: "pirouette aims to identify when assuming standard priors for the 
species tree leads to incorrect inference if one believes more complex 
diversification models are operating than can be currently accommodated in 
