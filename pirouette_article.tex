...
\new{
	...
	(usually extant, but extinction events and/or time-stamped data can also be added), 
...
\new{
	...
	viewing diversification as a branching process), while being computationally fast.
}
...
\new{
	...
	by writing a BEAST2 plugin that contains the likelihood formula of a phylogeny under the novel diversification model, 
	...
	Yule model [\cite{heled2015calibrated}],
	  ...
	  Fossilized BD process [\cite{gavryushkina2014bayesian}], and
	  BD SIR [\cite{kuhnert2014simultaneous}].
}
...
\new{
	...
	and their descendants [\cite{etienne2012conceptual, rabosky2014automatic, alfaro2009nine, laudanno2020sls}].
	...
	[\cite{maddison2007estimating, fitzjohn2012diversitree, herrera2018detecting}].
}
...
\rampal{
	This only increases the confusion; maybe replace by baseline model everywhere
}
\richel{
	I feel the new wording below is good enough. Replacing 'generative'
	by 'baseline' add to confusion when the generative model is the
	generative model. Giovanni, what do you think?
}
\rampal{
	This explanation again does not resolve the confusion. 
	You have a new model with which you generate a tree and the alignment. 
	In the pirouette paper this is DD. You could call this the generative model. 
	You also create a twin tree using a standard tree prior, e.g. BD. 
	Then in inference you use a tree prior, e.g. Yule. I think what you mean 
	is that Yule is not the one used for the twin tree, 
	and is therefore not the generative tree model. 
	But this is far from clear as it could also refer to the DD model. 
	Perhaps if you replace by "generative model for the twin tree", 
	then it is fine. Note that comparison model or baseline model 
	are NOT synonyms.
}
\rampal{
	Not synonyms for the generative tree model for the twin tree, I mean.
}
\richel{
	Giovanni, could you process this feedback?
}
\new{
  ...
  'generative tree model' serves as a baseline or comparison model.
}
...
\new{
  ...
  mononucleotide blocks (a block of length $n$ is a sequence in which the 
  same character is repeated $n$ times), which helps interpreting the 
  resulting alignment.
}
...
supplementary materials, section \ref{subsec:twinning} for more details).
...
\giovanni{
  Do we need the library calls in the listing?
}
\richel{
  I simplified the code and only call pirouette now. 
  I think this is cleaner then using a scope operator.
}
...
\new{
  ...
  the twinning process should be modified accordingly.
}
\new{
...
\giovanni{
	I don't understand this sentence: "pirouette aims to identify when assuming standard priors for the 
	...
	inference". Maybe shortening it, it can become clearer
}
\richel{
	Reworded, thanks!
}
