...
allowed to build phylogenetic trees 
...
evolve.
Such tools have been increasingly exploited to hypothesize and test what are the main drivers and modes for diversification processes. 
...
or even a combination of multiple concealed and observable states [\cite{herrera2018detecting}]. \giovanni{we might need to polish this list a little bit}
...
which provides the probability to realize a process given the parameters.
By maximizing it, it is possible 
to estimate what are the best values for model parameters. 
...
Diversification models are getting more complex over time. This is facilitated by the increase in computing power due to the advancement of technology.
Increased complexity made possible the exploration of new models encompassing mechanisms that are not yet considered by previous diversification models. An example of this is the multiple-birth model [Laudanno et al., in preparation],
...
Therefore desirable models have to be able to disentangle the intrinsic complexity of a system, highlight the main reasons of its behaviour and avoid unnecessary complexity. 
...
For this reason we believe it is important to have techniques to quantitatively assess the advancement provided by the introduction of new models, in comparison to the existing ones.
...
To achieve this, one possible experiment is to simulate a phylogeny according to the mechanisms proposed by the new diversification model, then simulate its alignment and measure how well current inference models perform at retrieving the original phylogeny.
...
\giovanni{I think we should mention the fact that BEAST2 allows to implement customized tree priors more complicated than the simple yule and bd. These tree priors, in the end, are what we call likelihood in our models, so it's pivotal in proposing our argument, I think.}
...
makes from a given reconstructed phylogeny \giovanni{We should mention here that the phylogenies that are meant to be submitted to pirouette are simulated phylogenies generated according to the model we want to test}. For this measurement, two models need to be specified: a generative and an inference model. 
...
an expected time when two or more taxa's \giovanni{I have the feeling that taxa should be plural here. Should we mention that the group must be monophyletic?} ancestors diversified. 
...
documentation.
\giovanni{it could be nice to add a listing with simple commands that refer to the most useful examples}
...
\richel{Only if the journal requires this}
