...
Due to this, effective but computationally expensive tree priors are not viable.
...
\richel{
  I think this is a useful paragraph, adding some depth to this paper,
...
These errors are unavoidably accumulated in the \verb;pirouette; pipeline. 
All the factors that can cause such errors are: the branching 
...
\giovanni{Can we show everything at the same time with standard histograms?}
. 
...
of the inference model along with site and clock models used to simulate the 
...
\richel{Yes, as we don't know anything about the process underlying the tree}
...
Since in the twin pipeline there is coherence between the inference model and the generative model, 
