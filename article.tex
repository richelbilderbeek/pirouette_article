...
be selected if it is the best model \giovanni{then why do you need the "run\_if" option?}.
...
\giovanni{How is the MCMC topic connected to what previously said?} An MCMC needs to 'warm up', as initially it will sample
...
site model $\mathit{s_{m}}$ (in this context: a nucleotide substitution model \giovanni{We already specified this in 2.1, when explaining the first step. Do we need to repeat it?}) 
...
The output is shown in figure~\ref{fig:example_4}. The alignment
...
a Yule tree of low (actually: zero) likelihood \giovanni{What do you mean by zero likelihood? This word might have a heavy meaning int his context...}, i.e. the one already used in ERQs 2-4, by using the code shown in listing~\ref{lst:unknown_phylogeny}.
