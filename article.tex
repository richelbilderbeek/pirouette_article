...
birth-death [Nee et al., 19..], time-dependent [?], diversity 
dependent [Etienne \& ?, approx 2012], protracted 
birth-death [Rosindell \& Etienne, 2011][Etienne \& Rosindell, 2012],
multiple-birth death [Laudanno, Bilderbeek \& Etienne, in preparation],
a rate shifting birth-death model [the SLS model. Laudanno \& Etienne, in preparation]
punctuated equilibrium aka early burst [?Harmon],
dependent on a binary trait [BISSE model, ?Maddison], 
continuous trait [HISSE model],
concealed state [?MUSSE] or a combination of all [secSSE model, Leonel].
...
is the multiple-birth model [Laudanno, Bilderbeek \& Etienne, in preparation], 
that is the first model to allow speciation to co-occur.

An open question for each new diversification model is if the increased
complexity is worth the extra computational resources. There are many
...
BEAST2 [X and Bouckaert et al., ????]. With \verb;pirouette;, one
...
makes from a given phylogeny of which full information is provided.
This involves multiple steps, described in detail below.
...
but also an custom error statistic is supplied, based
...
\end{lstlisting}
\giovanni{is "usethis" supposed to be "devtools"?}
\richel{"usethis" is the next "devtools", "devtools" will be deprecated in the future}
...
three elements: a site model, a clock
