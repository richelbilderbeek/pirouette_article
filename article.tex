...
\begin{figure}
  ...
  \caption{
    ...
    which is compared to the true phylogeny (1a) \giovanni{this makes me think that we might need an arrow going from 1a to 4a, maybe (and i say maybe) the arrow could be in red to highlight that the "true" nltt is in red too. BTW, the red line is not supervisible.} using
...
The second step selects an inference model, using the alignment \giovanni{Is it worth to add it in the figure, maybe on the side of the vertical pipeline? It could be something like an arrow moving horizontally to the box of inference model. Then the box could have another arrow pointing at the arrow connecting 2 and 3. If you think this is too complicated, and not easy to follow for the reader you can discard it.}.
...
the tree prior used by BEAST2 to produce a posterior \giovanni{is it true? is it Yule or BD? Please check}.
...
\giovanni{do we really want to use a function called "create\_ideal\_tree"?}
...
\begin{figure}[h]
  ...
  \caption{
    ...
    The x-axis specifies the tree type (which can be 'true' or 'twin'), the y-axis shows the error. \giovanni{This holds true for all the figures: if we want to add these numbers we need to specify what they represent in the caption.}
  }
  \label{fig:example_1}
\end{figure}
...
\caption{Results of the examples. 
  ...
  the median and range of the errors 
  is shown. Experiment 4 is split in two rows
  by either the candidate ('C') or generative model ('G'). \giovanni{How do you define range? Are we using the standard error? \url{https://en.wikipedia.org/wiki/Standard_error}}
}
\label{tab:results}
\end{table}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
