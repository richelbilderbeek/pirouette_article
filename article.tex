...
or even a combination of multiple concealed and 
observable states [\cite{herrera2018detecting}]. 
\giovanni{we might need to polish this list a little bit}
...
Such a new diversification model can then be added to BEAST2,
which already supplies multiple speciation models (among others, 
Yule and birth-death) and also allows to add 
new diversification models via the BEAST2 package system.
\richel{rework}
...
\giovanni{
  We should mention here that the phylogenies that are meant to be 
  submitted to pirouette are simulated phylogenies generated according 
  to the model we want to test
}. 
To be able to explain how we do this, we first give definitions
for generative model and inference model: 
...
an expected time when two or more taxa's 
\giovanni{
  I have the feeling that taxa should be plural here. 
  Should we mention that the group must be monophyletic?
