...
reduces to a standard birth-death model when speciation-completion rate approaches infinity.
...
model will be referred to as 'site model'. Supported site models are JC, HKY, TN and GTR. Only the strict
clock model is supported in this step.

The second step (Fig.~\ref{fig:pipeline}, 2a $\rightarrow$ 3a)
selects an experiment from a set of experiments. 
...
The fourth step quantifies the inference error made. First the burn-in is removed, i.e. the first phase of the Markov chain Monte Carlo (MCMC) run,
...
Assume that a non-standard generative prior $t_p = \mathit{P}$,
produces a tree $\tau_{\mathit{P}}$,
...
pipelines: the main pipeline uses the tree $\tau_{\mathit{P}}$ and a
