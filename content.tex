...
However, a novel tree prior $\mathit{p_{G}}$, by our definition, is non-standard, and hence 
usually it is not part of a standard inference 
...
prior, or picking the best of a set of tree priors. 
...
\verb;pirouette; means selecting the model with the highest evidence (also known as marginal likelihood). 

We show the procedure using \verb;pirouette; to answer the second research question.
...
\begin{figure}[H]
  ...
  \caption{
    ...
    when hand-picking a generative tree prior compared with the error obtained for the twin tree.
...
Rather than hand-picking a tree prior, 
one can select the best model from a set,
as in ERQ 2.
The resulting error distribution contains the full error accumulated in the pipeline, To quantify to what extent this error is due to the mismatch between 
the generative model $\mathit{G}$ and the selected inference model 
$\mathit{I}$, we can use twinning (see section \ref{subsec:twinning}: 'Twinning') to provide the baseline error.
...
We use again the default error measurement setup,
...
\begin{figure}[H]
  ...
  \caption{
    ERQ 4: the inference error made 
    when selecting the 'best' tree prior compared with the error obtained for the twin tree.
    Here, the 'twin' tree shows the baseline inference error,
    as measured on the twin tree.
  }
  \label{fig:example_4}
...
We use again the default error measurement parameters
...
\begin{figure}[H]
  ...
  \caption{
    ...
    each in relationship to the background error measured from the twin tree.
  }
  \label{fig:example_5}
\end{figure}
...
were obtained from thousands of replicates.
