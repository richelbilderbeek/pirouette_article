...
even when the generating model matches the assumed inference model
...
any differences will be due to the fact that true and twin phylogenies 
are realizations of different processes: one (possibly) non-standard, $p_G$, 
...
has been simulated, the candidate model with the highest evidence will be
...
if the generative model picked, has been a good choice.
...
we can see that the error distributions of the generative model
...
If this is indeed the case, one can implement 
