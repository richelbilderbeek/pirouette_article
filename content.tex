\begin{abstract}
  ...
  \textbf{1. }
    ...
    \iffalse
    This will inevitably introduce a bias on the inference, due to the choice of the tree prior. A way to estimate this error is to choose an inference tree prior different from a known generative prior.
    \fi
    Question is, how accurate the tree estimation is when the real macroevolutionary processes are substantially different from those assumed in the tree prior. \\
    \textbf{2. }
...
diversification models are available as a BEAST2 package.
...
prior choice. For example, recently Sarver and colleagues, [\cite{sarver2019choice}] 
showed that the choice of tree prior does not 
substantially affect phylogenetic inferences of diversification rates.
This study shows how current diversification models compare to one another, 
but they do not help to assess the importance of a new tree prior.
...
One such example is the diversity-dependent (DD) tree 
model [\cite{DDD, etienne2011diversity}]:
the DD model is a BD model with a speciation rate that is dependent 
on the number of species and a carrying capacity. For a carrying capacity
of infinity, the DD model reduces to a BD model.
...
JC is the default nucleotide substitution mode (NSM),
...
one can specify an inference model as such.
If the tree model is unknown or non-standard, one can pick
an (assumed) closest standard inference model.
...
Similar to other experiments, \verb;pirouette; allows to do
...
matches the true alignment. The total number of mutations is defines
...
but may be insure which one to pick. To account for this, the user can
specify a set of candidate inference models. After a true alignment
has been simulated, the candidate model with the highest 
evidence (a.k.a. marginal likelihood) will be
used in another inference run, resulting in another error distribution.
...
the twin alignment, this model is also used to create another error
distribution. Thanks to the use of candidate models, one can check
if the assumed generative model, has been a good choice.
...
by the non-standard diversity-dependent (DD) tree model \cite{DDD, etienne2011diversity},
...
and six taxa, to keep the figure more readable.
...
the error distribution shown in Figure \label{fig:example_30}
is produced.
...
Focussing on the upper panel of Figure \ref{fig:example_30},
...
compared to the assumed generative model, confirming that the 
picked generative model was a good choice.
...
model assumes that mutation rates differ between species, where the
alignment is simulated with equal mutation rates for all species.
The BD model is indeed closer to the DD model than Yule, as both BD and DD
model allow for extinction, where Yule does not.
...
tree model possible.
In principle any other standard tree model can be assume, 
...
of multiple trees, generated by the novel tree model, should be used.
Also, we think that judging a new tree model by its recovery when using
a standard tree prior is an oversimplification: a new tree model may
still tell us something about an evolutionary aspect such as 
a trait or geography. \verb;pirouette;'s results encourage
to convert this novel tree model into a standard tree prior, 
so that the inference is (as predicted to be) improved strongly.

Like most phylogenetic experiments, the setup of \verb;pirouette;
is determined by many arbitrary choices. A prime example is the
length of simulated DNA sequence. We predict that the inference error
decreases for longer DNA sequences, but we only investigated this
superficially (see the supplementary materials). The strength
of \verb;pirouette; is, however, to supply a pipeline that is uniform
regardless these choices.

Interpreting the results of \verb;pirouette; remains a subject 
of study. \verb;pirouette; can quantify these, which is a good
first step, but does not give a unequivocal answer to the question 
if the inference error is profound. Using the nLTT statistic is
a promising starting, as it can compare any two trees and 
results in an error distribution of known range. If a researcher
would be more interested in other aspects such as tree balance,
\verb;pirouette; allows for this.

One interesting avenue to find out some guidelines is to compare
the results of \verb;pirouette; with those from a recent study 
by Duchêne and colleagues [\cite{duchene2018phylodynamic}],
where they also created a method to assess the adequacy of a tree model
on empirical trees. A feature of their setup, is that they bypass
the simulation of alignments, which definitely has its advantages,
for example, in terms of run-time speed and the need for less assumptions.
On the other hand, \verb;pirouette; uniquely allows to estimate the
adequacy of a tree prior in combination with novel clock and site models.
...
tailored to the novel tree model, is needed. 
If this is indeed the case, one has a reason
to implement the novel tree prior as an addition to 
