...
We show the usage of \verb;pirouette; by a story that introduces
its features gradually. First we measure the Bayesian inference error
when we have a phylogeny with a known and standard tree prior,
to get an idea of the baseline error.
Second, we'll additionally measure the inference error made by a
best candidate model, to establish that the true tree prior is indeed the best.
Lastly, we will seperate the baseline error and complete error in
a tree generated by an unknown or non-standard tree prior, 
to quantify the impact the tree prior has in Bayesian inference.
...
can answer: "What is the inference error made on phylogenies
...
In this example, the root sequence consists of four blocks of 250 mononucleotides each, 
...
the function arguments can be safely omitted: we just show these explicitly for clarity.
...
Here we use the default error measurement
...
We now have create the alignment parameters,
our experiments and our error measure parameters.
These objects need to be bundled in a bigger
\verb;pirouette; parameter structure, 
...
We call it 'twinning' and we use it in the last part of this example.
...
However, a novel tree prior $\mathit{p_{G}}$, by our definition, 
is non-standard, and hence not part of a standard inference 
model (i.e. an inference model using a standard tree prior).
In such a case, the question is which standard tree prior should be 
used in the inference.
In this example, we re-use our hand-picked inference model,
but additionally pick a best inference model from a set of inference models.
With 'best' inference model, we mean the model with the highest 
evidence (also known as marginal likelihood), based on the alignment.
We show the procedure using \verb;pirouette; to answer a second research 
question: "What is the inference error made on a novel phylogeny when
using the best inference model, in comparison to a hand-picked model?
...
We will use the same tree as generated in \ref{lst:create_yule_tree},
as well as the same alignment parameters as shown in 
Listing~\ref{lst:create_alignment_params}.

We do need to specify a different set of experiments: we
need to state that we already have an experiment for
the generative model, as well as that we want all other inference
models to compete. We call the competing models 'candidate models',
as only the best will be used in the actual inference.
As model selection is commonly performed on the full list of available 
candidate models, \verb;pirouette; has a dedicated function for this choice:
...
4 site models, 2 clock models and 5 tree priors. All we need to add is
to exclude the inference model in the generative experiment:
...
We combine the generative and all candidate models into one set of experiments:
...
We can now create the complete
...
as shown in Figure~\ref{fig:example_6}.

\begin{figure}[H]
  \includegraphics[width=\textwidth]{example_6/errors.png}
  \caption{
    The inference error
    for both the generative and best candidate inference models.
  }
  \label{fig:example_6}
\end{figure}
...
Up until now, we measured the inference error on a tree
of which we knew the (standard) tree prior. The goal of \verb;pirouette;
is to measure the impact of a novel tree prior.
To do so, in this example, we'll use a tree generated by a non-standard
tree prior, of which we assume is closely related to the Yule tree prior.
To measure the impact of the tree prior, we measure both the
baseline error (that serves as a control) 
and the full inference error: their difference shows
the impact of the tree prior.
To do so, we use a twin tree (see Subsection~\ref{subsec:twinning}) for
both the generative and best candidate model.
The research question this example answers is:
"What is the inference error made from a phylogeny,
for both a generative model and a best candidate model, 
...
we re-use the alignment parameters (Listing~\ref{lst:create_alignment_params}),
...
using the most simple generative tree prior possible.
...
it constitutes a quantitative assessment 
...
one could rightly implement their novel tree prior as an addition to 
