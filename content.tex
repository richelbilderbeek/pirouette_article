...
\begin{figure}
  ...
  \caption{
    ...
    phylogeny (1a) using the twin tree prior $\mathit{p_{t}}$, 
    selected among standard tree priors; 
    the default option is the standard birth-death prior.
    ...
    The twin alignment has the same amount of mutations as the original alignment.
    The twin pipeline mimics the procedure of the main pipeline, 
...
produced by a (possibly non-standard) generative prior, 
...
\end{align}
To do so, a standard tree prior $p_T$ for the twin tree must be specified. 
The default option here is the standard birth-death prior.
It is then possible to use the likelihood function 
$L_{\mathit{T}}$ defined by such tree prior to find the parameters $\theta^{*}_{\mathit{T}}$ 
...
The second step, $T_2$m of the twinning process simulates the twin alignment. 
To do so, we use the same clock model, site model and mutation rate 
...
must be the same as in the true alignment.
...
The twin pipeline provides this minimum error possible,
...
the tree prior chosen for the twin inference will be 
exactly the same used to generate the tree.
In the main pipeline, as the tree prior $p_{G}$ is non-standard, 
it cannot be used in inference.
If, instead, the chosen model type is "candidate", 
the twin tree prior will be included in the pool of examined models 
...
In fact, a single phylogeny cannot be considered as fully representative of the prior. 
For this reason multiple phylogenies, 
as well as an equal amount of twins, 
must be considered. 
...
of the new tree prior respect to the baseline error.
...
We start from the tree generated by a non-standard (and unknown) tree prior, 
