...
example research questions (ERQ) that \verb;pirouette; can answer, from simple questions to complex ones.
...
gen_experiment <- create_experiment(
...
experiments <- list(gen_experiment)
\end{lstlisting}
...
\begin{figure}[H]
  ...
  \caption{
    ERQ 1: the inference error made 
...
and hence usually it is not part of a standard inference 
model (i.e. an inference model using a standard tree prior).
In such a case, the question is what standard tree prior should be 
used in the inference.
There are three ways to deal with this problem: hand-picking a tree
prior (as we did above), or picking the best of a set of tree priors, 
or to do both. 
...
the novel tree prior is most similar to a specific standard tree prior.
Instead of hand-picking, the best inference model from a list of candidate 
models can also be used in inference. Selecting the 'best' model in
\verb;pirouette; means selecting the model with the highest 
...
In the second step, we deviate from the answer to ERQ 1 by providing a 
...
to run the actual full inference. According to the specified $\mathit{C}$, 
...
cand_experiments <- create_all_experiments(
...
Up until now, we measured the baseline error of the \verb;pirouette; method,
...
by using a twin tree (see subsection~\ref{subsec:twinning}) for
...
Most of the settings for this example are
the same as in ERQ 2: we will re-use the phylogeny as 
before (Listing~\ref{lst:create_yule_tree}), 
as well as the alignment parameters (Listing~\ref{lst:create_alignment_params}).
Also the experiments are the same as in 
Listing~\ref{lst:create_all_experiments}.
...
to add now}
\rampal{did I say that? I think it's OK not to do it in the paper, but perhaps it can be noted.}
...
In all ERQs we used the most simple generative tree prior possible,
...
a minimum noise level assessment in a larger experiment.  
